  ...
  var width = parseInt(d3.select('#map-median-income').style('width')),
  mapRatio = 0.51,
...
var colour =  d3.scaleThreshold()
                .domain([75000,100000,125000,150000,175000,200000])
                // .range(['#fff8b5', '#ffe689', '#ffd257', '#ffbe00', '#b09c00', '#647900', '#005600']);
                .range(['#fff8b5', '#f5d264', '#cfb437', '#969d33', '#458b36', '#28701d', '#005600']);


//Create SVG element

var svg = d3.select("#map-median-income")
      .append("svg")
      .attr("width", width)
      .attr("height", height);
...
d3.csv("./data/median-income.csv", function(error, data) {
    
      console.log(data)
   data.forEach(function(d) {
       // varByName.set(d.id, d.covid);
       varByName.set(d.GeoUID, +d["v_CA21_906: Median total income of household in 2020 ($)"]);
  ...
  d3.selectAll(".ward")
    ...
    .on("mouseover", function(d) {
          ...
          d3.select("#tooltip")
                  .select("#value")
                  .html(function(){
                    if ( isNaN(varByName.get(d.properties.id)) || varByName.get(d.properties.id) <= 0) {
                        return "<p class='ttvaluelabel'>Area: " 
                        + (d.properties.id) 
                        +  "</p><p class='ttvaluelabel'>Median household income:</p><p class='ttvalue'>" 
                        // + d.properties.GeoUID
...
width = parseInt(d3.select('#map-median-income').style('width')),
