    ...
    var width = parseInt(d3.select('#map-age-change').style('width')),
    mapRatio = 0.51,
  ...
  var colour =  d3.scaleThreshold()
                  .domain([-20,-10,-5,0,5,10])
                  // .range(['#fff8b5', '#ffe689', '#ffd257', '#ffbe00', '#b09c00', '#647900', '#005600']);
                  .range(['#a7c85b', '#c4d890', '#dfe8c4', '#D0D0D0', '#b9b7de', '#777ac4', '#1b43a9']);
  
  
  //Create SVG element
  
  var svg = d3.select("#map-age-change")
        .append("svg")
        .attr("width", width)
        .attr("height", height);
  ...
  d3.csv("./data/data-age-2016-and-2021.csv", function(error, data) {
      
     data.forEach(function(d) {
         // varByName.set(d.id, d.covid);
         if ( d.GeoUID.length == 7 ) {
             varByName.set(d.GeoUID + ".00", +d["age difference 2016-2021"]);
         } else if ( d.GeoUID.length == 9 ) {
            varByName.set(d.GeoUID + "0", +d["age difference 2016-2021"]);
         } else {
            varByName.set(d.GeoUID, +d["age difference 2016-2021"]);
         }
...
labels.append("text")
 ...
 labels.append("text")
            ...
            d3.select("#tooltip")
                    ...
                    .html(function(){
                      if ( isNaN(varByName.get(d.properties.id)) ) {
                          ...
                          + (d.properties.id) 
                          +  "</p><p class='ttvaluelabel'>Average age difference:</p><p class='ttvalue'>" 
                          // + d.properties.GeoUID
  ...
  width = parseInt(d3.select('#map-age-change').style('width')),
