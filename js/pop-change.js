    ...
    var width = parseInt(d3.select('#map-pop-change').style('width')),
    mapRatio = 0.51,
  ...
  var colour =  d3.scaleThreshold()
                  .domain([-15,-10,-2.5,0,2.5,10,15])
                  // .range(['#fff8b5', '#ffe689', '#ffd257', '#ffbe00', '#b09c00', '#647900', '#005600']);
                //   .range(['#ff005b', '#ff8090', '#ffbfc3', '#D0D0D0', '#e9c4a7', '#d0925b', '#b16200']);
                  .range(['#c7b944', '#d6c873', '#e3d89f', '#efe8cb', '#fecbba', '#fa9e7e', '#ef7045', '#de3900']);
  
  
  //Create SVG element
  
  var svg = d3.select("#map-pop-change")
        .append("svg")
        .attr("width", width)
        .attr("height", height);
  ...
  d3.csv("./data/data-pop-change-2016-to-2021.csv", function(error, data) {
     ...
     data.forEach(function(d) {
         ...
         if ( d.GeoUID.length == 7 ) {
             varByName.set(d.GeoUID + ".00", +d["v_CA21_3: Population percentage change, 2016 to 2021"]);
         } else if ( d.GeoUID.length == 9 ) {
            varByName.set(d.GeoUID + "0", +d["v_CA21_3: Population percentage change, 2016 to 2021"]);
         } else {
            varByName.set(d.GeoUID, +d["v_CA21_3: Population percentage change, 2016 to 2021"]);
         }
  ...
  d3.json("./data/toronto-census-tracts-CT.json", function(error,json) {
    ...
    g.selectAll("path")
       ...
       .attr("fill", function(d) { 
              if ( isNaN(varByName.get(d.properties.id)) ){
                return "#F8F8F8"
            } else {
            ...
            d3.select("#tooltip")
                    ...
                    .html(function(){
                      if ( isNaN(varByName.get(d.properties.id)) ) {
                          ...
                          + (d.properties.id) 
                          +  "</p><p class='ttvaluelabel'>Population percent change</p><p class='ttvalue'>" 
                          // + d.properties.GeoUID
                          ...
                          let addPlusSign = ''
                          if (varByName.get(d.properties.id) > 0) { 
                            addPlusSign = '+'
                          }
                          return "<p class='ttvaluelabel'>Area: " 
                          + (d.properties.id) 
                          ...
                          // + d.properties.GeoUID
                          // + "</p><p class='ttvaluelabel'>Average prices:</p><p class='ttvalue'>$" 
                          + addPlusSign + d3.format(',')(varByName.get(d.properties.id)) + "%"
                          + "</p>";
                      }
                    });
  ...
  width = parseInt(d3.select('#map-pop-change').style('width')),
