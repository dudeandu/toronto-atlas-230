  ...
  var width = parseInt(d3.select('#map-generations').style('width')),
  mapRatio = 0.51,
...
var colour =  d3.scaleThreshold()
                // .domain([0,.10,.45,.50,.55,.9,1])
                // .domain([0.1,.25,.40,.50,.55,.7,1])
                .domain([0.20,.30,.40,.55,60,.75,.9])
                // .range([,"#6c3a0d","#a6611a","#dfc27d","#f5f5f5","#80cdc1","#018571","#01665e"]);
                .range(['#ecffc9', '#cbe5a8', '#abcc87', '#8bb468', '#6b9c48', '#4b8429', '#286d00']);


//Create SVG element

var svg = d3.select("#map-generations")
      .append("svg")
      .attr("width", width)
      .attr("height", height);
...
d3.csv("./data/demographics.csv", function(error, data) {
   ...
   data.forEach(function(d) {
       ...
       varByName.set(d.GeoUID, +d["v_CA21_4818: Total - Generation status for the population in private households"]);
...
d3.json("./data/toronto-census-tracts-CT.json", function(error,json) {
  ...
  g.selectAll("path")
     ...
     .attr("fill", function(d) { 
         if (isNaN(var2ByName.get(d.properties.id)/varByName.get(d.properties.id))) {            
                return "#F8F8F8"
            } else {
                return colour(var2ByName.get(d.properties.id)/varByName.get(d.properties.id)); 
            }
        })
        .attr("opacity", 0.9);
          ...
          d3.select("#tooltip")
                  ...
                  .html(function(){
                    if ( isNaN(var2ByName.get(d.properties.id)/varByName.get(d.properties.id)) ) {
                        ...
                        + (d.properties.id) 
                        +  "</p><p class='ttvaluelabel'>First generation Canadians:</p><p class='ttvalue'>" 
                        // + d.properties.GeoUID
                        ...
                        // + d.properties.GeoUID
                        // + "</p><p class='ttvaluelabel'>Average prices:</p><p class='ttvalue'>$" 
                        + d3.format('.2%')(var2ByName.get(d.properties.id)/varByName.get(d.properties.id))
                        + "</p>";
                    }
                  });
...
width = parseInt(d3.select('#map-generations').style('width')),
